...
public protocol TimelineComponentTarget: class {
     ...
     */
    func loadInRange(_ range: Range<Int>, completionBlock: @escaping ([ContentType]?) -> Void)
}
...
open class TimelineComponent <T: Equatable, S: TimelineComponentTarget> : Refreshable where S.ContentType == T {
    ...
    open func removeObject(_ object: T) {
        ...
        let newRange:Range<Int> = self.currentRange.lowerBound..<self.currentRange.upperBound
    ...
    open func loadInitialIfRequired() {
        ...
        if (content == []) {
            target!.loadInRange(target!.defaultRange) { posts in
                self.content = posts ?? []
                let newRange:Range<Int> = self.currentRange.lowerBound..<self.content.count+1
                self.currentRange = newRange
